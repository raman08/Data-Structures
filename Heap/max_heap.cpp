...
// Class contaning all methods required in max-heap.
class Max_Heap {
...
private:
	...
	void shift_down(int child) {
		...
		// If left child index is smaller the heap size and left child value is greater the the given value
		...
		// If right child index is smaller the heap size and right child value is greater the the given value
...
public:
	...
	void remove(int data) {
		...
		if (child >= 0 && child < count) {
			...
			heap[child] = INT_MAX; // Replacing the data with maximun value of the integer.

			shift_up(child); // Shifting up to regrrange the heap

			extract_max(); // Extracting the maximin value from heap. (We ourself given the node to be removed the maximun value.)
